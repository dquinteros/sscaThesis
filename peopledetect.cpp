...
int writescore(std::vector<int> gtitems, char* filename, float** img, int percent);
...
std::vector<float> v  = HOGDescriptor::getDefaultPeopleDetector();
...
{   
    ...
    float** imagen = new float *[ant_info.at(0)];
    for (int i = 0; i < ant_info.at(0); i++)
        imagen[i] = new float [ant_info.at(1)];
...
/*obtiene información de la anotaciones*/
   ...
   int writescore(std::vector<int> gtitems, char* filename, float** img, int percent){
    ...
    for(int i=0;i<gtitems.at(3);i++){
        ofstream myfile;
        string file = boost::lexical_cast<string>("out/")+name+boost::lexical_cast<string>("_")+boost::lexical_cast<string>(i+1)+boost::lexical_cast<string>(".out");
        std::cout << file << std::endl;
        myfile.open (file.c_str());        
        int item_x = gtitems.at(4+2*i);
        int item_y = gtitems.at(5+2*i);
        myfile << "# name: MAT\n# type: matrix\n# rows: " << xbound*2 << "\n# columns: " << ybound*2 << "\n";
        std::cout << "(" << item_x << "," << item_y << ") = " << img[item_x][item_y] << std::endl;
        for (int x = item_x-xbound; x < item_y+xbound; x++){
            for (int y = item_y-ybound; y < item_y+ybound; y++){                    
                myfile << " " << img[x][y];
            }
            myfile <<"\n";
        }  
